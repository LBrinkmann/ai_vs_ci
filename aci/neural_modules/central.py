...
class CentralAgent(nn.Module):
    def __init__(self, view_size, n_actions, hidden_size, n_agents, rnn1, linear2, device, **kwargs):
        ...
        if rnn1:
            self.rnn1 = nn.GRU(input_size=hidden_size, hidden_size=hidden_size, batch_first=True)
        else:
            self.rnn1 = None
        if linear2:
            ...
            self.linear2 = None
        self.linear3 = nn.Linear(in_features=hidden_size, out_features=n_actions*n_agents)
